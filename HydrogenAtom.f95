...
!Legendre      
       ...
       Function why(m,l)
              ...
              why = ((-1)**m)*SQRT(((2*l+1)/(4*pi))*(gamma(l-m+1)/gamma(l+m+1)))
       End Function
...
3             IF(ecl .LE. 19998) THEN          
              ...
              If (ecl .LT. 19998 .AND. ecl .GT. 1) Then
              
              rho = pi*((0.5D0*n*xa)**2.0D0)*((casio(n,l,xa)*ya*why(m,l)*eya)**2.0D0)*SQRT(1-exa**2.0D0)
              T = rho*0.5D0*n*h*ABS(ACOS(exa+eh)-ACOS(exa-eh)) + T
              error = (2*ABS(T*(0.000005)/ya))**2+(2*ABS(T*(0.00005)/eya))**2 + error
...
!              End If
              ...
              Write(*,*) xmax,  xa*0.5*n, ET*a0**3, SQRT(error)*a0**3
6             Format(D8.5,D8.5,D8.5)             
